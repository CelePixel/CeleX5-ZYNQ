...
{
	//cout << "---------- size:" << m_pDataProcessThread->getDataProcessor5()->getIMUData(data) << endl;
...
int CeleX5::connectToZYNQServer(std::string address, unsigned short port)
{
	int state = m_pTcpSocket->connectToZYNQServer(address, port);
	if (-1 != state)
	{
		reset();
	//	setClockRate(100);
	//	setSensorFixedMode(CeleX5::Full_Optical_Flow_S_Mode);
	}
	return state;
}
...
bool CeleX5::readBinFileData()
{
	...
	while (true && m_pDataProcessThread->queueSize() < 20 && m_bIsPlayBack)
	{

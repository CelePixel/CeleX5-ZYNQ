...
//------------------------------------------------------
//-- MIPIDataReaderThread
//------------------------------------------------------
MIPIDataReaderThread::MIPIDataReaderThread(CeleX5* pSensor, const std::string &name)
	: XThread(name)
	, m_bPipeoutAllowed(false)
	, m_pCeleX5(pSensor)
{
...
MIPIDataReaderThread::~MIPIDataReaderThread()
{

}

void MIPIDataReaderThread::startReadData(bool bRead)
{
	m_bPipeoutAllowed = bRead;
}

void MIPIDataReaderThread::run()
{
	while (m_bRun)
	{
		//cout << "---------- DataReaderThread::run ----------" << endl;

...
void CelexRosCallBackNode::setCeleX5(CeleX5 *pcelex) {
  ...
  celex_->setClockRate(clock_rate_);
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "celex_zynq_callback");

  std::string ip_address_ = "192.168.1.11";
  ...
  if (NULL == pCelex_)
    return 0;
  pCelex_->openSensor(CeleX5::CeleX5_ZYNQ);
  pCelex_->connectToZYNQServer(ip_address_,port_);
